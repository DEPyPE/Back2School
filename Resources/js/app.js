...
polygonTemplate.events.on("hit", function(ev) {
    ...
    color_splited = DataActualCountry.color.split(',');
    color_background = color_splited[0] + ',' + color_splited[1] + ',' + color_splited[2] + ', 0.9)';

    $('#ShowCountryInformation').css({
        background: "linear-gradient("+color_background+" 0%, "+color_background+" 25%, rgba(255, 255, 255, 0.9) 25%, rgba(255, 255, 255, 0.9) 100%)"
    });

    $('#CountryNoConsider').css({
        background: "linear-gradient("+color_background+" 0%, "+color_background+" 50%, rgba(255, 255, 255, 0.9) 50%, rgba(255, 255, 255, 0.9) 100%)"
    });
    ...
    $('.nombreEstrategia').html(DataActualCountry.nombreEstrategia);
    $('.actividadesPresenciales').html(DataActualCountry.actividadesPresenciales);
    $('.aforosFiltrosSanitarios').html(DataActualCountry.aforosFiltrosSanitarios);

    var ReincoportationVoluntary = '';

    if( DataActualCountry.reincorporacionVoluntaria == 'SI' ){
        ReincoportationVoluntary = 'check_circle';
    } else if( DataActualCountry.reincorporacionVoluntaria == 'NO' ){
    ...
    if( DataActualCountry.reincorporacionGradual == 'SI' ){
        ReincoportationGradual = 'check_circle';
    } else if( DataActualCountry.reincorporacionGradual == 'NO' ){
    ...
    if( DataActualCountry.docentesVacunados == 'SI' ){
        DocentesVacunados = 'check_circle';
    } else if( DataActualCountry.docentesVacunados == 'NO' ){
        DocentesVacunados = 'cancel';
    }else{
        DocentesVacunados = 'info';
    }

    var AlumnosVacunados = '';

    if( DataActualCountry.alumnosVacunados == 'SI' ){
        AlumnosVacunados = 'check_circle';
    } else if( DataActualCountry.alumnosVacunados == 'NO' ){
    ...
    $('.alumnosVacunados         ').html("<i class='material-icons icon-color-"+AlumnosVacunados+"'>"+AlumnosVacunados+"</i>");

    var TypeSate = DataActualCountry.color;
    ...
    for(var i=0; i<DataActualCountry.referencias.length; i++){
